...
Every new MacOS update brings a new set of challenges. This is because the OS and the BIOS is tightly couped in Macs. MacOS updates often update the BIOS too (well MacOS doesn't have a concept of BIOS. The OS itself is responsible for the firmware)
...
The fingerprint sensor requires a driver. Linux is known to have bad support for fingerprint sensors. It isn't that important but it's just a quality of life improvement. Drivers are usually released by the hardware manufacturer themself. It's no surprise that Apple exactly 0 drivers for their hardware.The fingerprint sensor requires a driver. Linux is known to have bad support for fingerprint sensors. It isn't that important but it's just a quality of life improvement. Drivers are usually released by the hardware manufacturer themself. It's no surprise that Apple exactly 0 drivers for their hardware.
...
WiFi works as expected in Asahi Linux, however, the WiFi P2P protocol is not implemented at the driver level. One of my ideas to work around the external display problem is to use Miracast. There are a few linux implementations on Miracast protocol like `gnome-network-displays` and Miraclecast. These unfortunately do not work since the P2P protocol is not implemented. The WiFi Chip (that is BRCM4378) is capable of WiFi P2P but nor Apple or Broadcom have released linux drivers for this chip.
...
So this means we can play all games that run on Linux on a Macbook? 
Not so fast-

#### 2. Games

Running games on linux has been a pain for a long time. This is because most games are written for Windows. Windows has a lot of proprietary APIs that games use. The most popular one is DirectX. DirectX is a set of APIs that are used to interact with the GPU. DirectX is not available on Linux. This means that games that use DirectX cannot run on Linux. We need to translate DirectX calls to Vulkan calls and this is what `DXVK` does. DXVK is a compatibility layer that translates DirectX calls to Vulkan calls. This allows us to run DirectX games on Linux.
...
There are a few things I'd be missing too going back to MacOS. The freedom of customization is priceless. I run a clean SwayWM setup. This level of tiling window manager smoothness is not achievable on Mac (See [yabai](https://github.com/koekeishiya/yabai) and [Aerospace](https://github.com/nikitabobko/AeroSpace)) The joy of running Open Source Software is not present on MacOS. As many people call it, it's a walled garden. I can't mess with system files and figure out how operating systems work. Linux has taught me practically everything I know about operating systems and so much about the kernel, drivers, and how they work. 
...
I'm back again today and most of these things work! It's been like seeing a child grow up and the amount of progress being made so fast is extremely inspiring. It reinforces trust in the open source community and proves what humans can achieve when we put our minds together. 
